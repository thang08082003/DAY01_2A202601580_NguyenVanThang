...
def run_assistant(
    ...
    while True:
        ...
        turns += 1
        tokens_used += count_tokens(user_msg, model=OPENAI_MODEL)
        tokens_used += count_tokens(assistant_msg, model=OPENAI_MODEL)
        total_cost += estimate_cost(user_msg, assistant_msg, model=OPENAI_MODEL)[
            "total_cost"
        ]
